...
class GearmanWorker(GearmanConnectionManager):
    ...
    def register_task(self, task, callback_function):
        ...
        for command_handler in self.handler_to_connection_map.keys():
            command_handler.set_abilities(
                self.handler_initial_state['abilities'])
    ...
    def set_client_id(self, client_id):
        ...
        for command_handler in self.handler_to_connection_map.keys():
            command_handler.set_client_id(
                self.handler_initial_state['client_id'])
    ...
    def establish_worker_connections(self):
        ...
        for current_connection in self.randomized_connections:
            try:
                valid_connection = self.establish_connection(
                    current_connection)
    ...
    def handle_error(self, current_connection):
        ...
        the job lock"""
        current_handler = self.connection_to_handler_map.get(
            current_connection)
        if current_handler:
    ...
    def wait_until_updates_sent(
            self,
            multiple_gearman_jobs,
            poll_timeout=None):
        connection_set = set(
    ...
    #####################################################
    def create_job(self, command_handler, job_handle, task, unique, data, when_to_run):
    ...
    def set_job_lock(self, command_handler, lock):
        ...
        failed_unlock = bool(
            not lock and self.command_handler_holding_job_lock !=
            command_handler
        )
