...
from python3_gearman.constants import PRIORITY_NONE, PRIORITY_LOW, \
    PRIORITY_HIGH, JOB_UNKNOWN, JOB_PENDING
from python3_gearman.errors import ConnectionError, \
    ExceededConnectionAttempts, ServerUnavailable

gearman_logger = logging.getLogger(__name__)

# This number must be <= GEARMAN_UNIQUE_SIZE in
...
class GearmanClient(GearmanConnectionManager):
    ...
    def __init__(
            self,
            host_list=None,
            random_unique_bytes=RANDOM_UNIQUE_BYTES):
        ...
        self.request_to_rotating_connection_queue = compat.defaultdict(
            collections.deque)
    ...
    def submit_job(
        ...
        return python3_gearman.util.unlist(completed_job_list)
    ...
    def submit_multiple_jobs(
        ...
        """
        assert type(jobs_to_submit) in (
            list, tuple, set), "Expected multiple jobs, received 1?"
        ...
        requests_to_submit = [
            self._create_request_from_dictionary(
                job_info,
                background=background,
                max_retries=max_retries) for job_info in jobs_to_submit]
    ...
    def submit_multiple_requests(
        ...
        * Optionally blocks until jobs are all complete
        ...
        """
        assert type(job_requests) in (
            list, tuple, set), "Expected multiple job requests, received 1?"
        stopwatch = python3_gearman.util.Stopwatch(poll_timeout)

        # We should always wait until our job is accepted, this should be fast
        time_remaining = stopwatch.get_time_remaining()
        processed_requests = self.wait_until_jobs_accepted(
            job_requests, poll_timeout=time_remaining)

        # Optionally, we'll allow a user to wait until all jobs are complete
        # with the same poll_timeout
        time_remaining = stopwatch.get_time_remaining()
        if wait_until_complete and bool(time_remaining != 0.0):
            ...
            for current_request in processed_requests:
                self.request_to_rotating_connection_queue.pop(
                    current_request, None)

        return processed_requests

    def wait_until_jobs_accepted(self, job_requests, poll_timeout=None):
        """Go into a select loop until all our jobs have moved to
        STATE_PENDING"""
        assert type(job_requests) in (
            list, tuple, set), "Expected multiple job requests, received 1?"
        ...
        # Poll until we know we've gotten acknowledgement that our job's
        # been accepted
        # If our connection fails while we're waiting for it to be accepted,
        # automatically retry right here
        def continue_while_jobs_pending(any_activity):
            ...
            return compat.any(is_request_pending(current_request)
                              for current_request in job_requests)

        self.poll_connections_until_stopped(
            self.connection_list,
            continue_while_jobs_pending,
    ...
    def wait_until_jobs_completed(self, job_requests, poll_timeout=None):
        ...
        def continue_while_jobs_incomplete(any_activity):
            for current_request in job_requests:
                if (is_request_incomplete(current_request) and
    ...
    def get_job_status(self, current_request, poll_timeout=None):
        ...
        request_list = self.get_job_statuses(
            [current_request], poll_timeout=poll_timeout)
        return python3_gearman.util.unlist(request_list)
    ...
    def get_job_statuses(self, job_requests, poll_timeout=None):
        ...
        for current_request in job_requests:
            current_request.status[
                'last_time_received'] = (
                current_request.status.get('time_received')
            )

            current_connection = current_request.job.connection
            current_command_handler = self.connection_to_handler_map[
                current_connection]

            current_command_handler.send_get_status_of_job(current_request)

        return self.wait_until_job_statuses_received(
            job_requests, poll_timeout=poll_timeout)

    def wait_until_job_statuses_received(
            self, job_requests, poll_timeout=None):
        """Go into a select loop until we received statuses on all our
        requests"""
        assert type(job_requests) in (
            list, tuple, set), "Expected multiple job requests, received 1?"
        ...
        def is_status_not_updated(current_request):
            ...
            return bool(current_status.get('time_received') ==
        ...
        def continue_while_status_not_updated(any_activity):
            for current_request in job_requests:
                if (is_status_not_updated(current_request) and
                        (current_request.state != JOB_UNKNOWN)):
                    return True
        ...
        self.poll_connections_until_stopped(
            self.connection_list,
            continue_while_status_not_updated,
            timeout=poll_timeout)
    ...
    def _create_request_from_dictionary(
            self, job_info, background=False, max_retries=0):
        """Takes a dictionary with fields  {'task': task, 'unique': unique,
        'data': data, 'priority': priority, 'background': background}"""
        # Make sure we have a unique identifier for ALL our tasks
        job_unique = job_info.get('unique')
        if job_unique == '-':
            job_unique = job_info['data']
        elif not job_unique:
            job_unique = binascii.hexlify(os.urandom(
                self.random_unique_bytes)).decode('utf-8')
        ...
        current_request = self.job_request_class(
            current_job,
