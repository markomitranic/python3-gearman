...
from python3_gearman.errors import InvalidWorkerState
from python3_gearman.protocol import GEARMAN_COMMAND_PRE_SLEEP, \
...
class GearmanWorkerCommandHandler(GearmanCommandHandler):
    ...
    def send_job_status(self, current_job, numerator, denominator):
        assert type(numerator) in (
            int, float), 'Numerator must be a numeric value'
    ...
    def send_job_exception(self, current_job, data):
        ...
        # http://groups.google.com/group/gearman/browse_thread/thread
        # /5c91acc31bd10688/529e586405ed37fe
        #
        self.send_command(
    ...
    def _release_job_lock(self):
        if not self.connection_manager.set_job_lock(self, lock=False):
            raise InvalidWorkerState(
                "Unable to release job lock for %r" %
                self)
    ...
    def recv_noop(self):
        ...
        AWAITING_JOB -> AWAITING_JOB ::
            Noop transition, we're already awaiting a job
        SLEEP -> AWAKE -> AWAITING_JOB ::
            Transition if we can acquire the worker job lock
        SLEEP -> AWAKE -> SLEEP        ::
            Transition if we can NOT acquire a worker job lock
        """
    ...
    def recv_no_job(self):
        ...
        AWAITING_JOB -> SLEEP ::
            Always transition to sleep if we have nothing to do
        """
    ...
    def recv_job_assign_uniq(self, job_handle, task, unique, data, when_to_run=None):
        ...
        AWAITING_JOB -> EXECUTE_JOB -> SLEEP ::
            Always transition once we're given a job
        """
