...
from python3_gearman.constants import JOB_UNKNOWN, JOB_PENDING, JOB_CREATED, \
    JOB_FAILED, JOB_COMPLETE
from python3_gearman.errors import InvalidClientState
from python3_gearman.protocol import GEARMAN_COMMAND_GET_STATUS, \
...
class GearmanClientCommandHandler(GearmanCommandHandler):
    ...
    def _register_request(self, current_request):
        self.handle_to_request_map[
            current_request.job.handle] = current_request
    ...
    def recv_job_created(self, job_handle):
        if not self.requests_awaiting_handles:
            raise InvalidClientState(
                'Received a job_handle with no pending requests')
    ...
    def recv_work_status(self, job_handle, numerator, denominator):
        ...
        # The protocol spec is ambiguous as to what type the numerator and
        # denominator is...
    ...
    def recv_status_res(
        ...
        # If the server doesn't know about this request, we no longer need to
        # track it
        if not job_known:
            self._unregister_request(current_request)
